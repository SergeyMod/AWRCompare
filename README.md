...
Веб-приложение для автоматического сравнения отчетов о производительности баз данных Oracle (AWR) и PostgreSQL (pg_profile).

## Описание

Данное веб-приложение предназначено для автоматизации процесса сравнения отчетов о производительности баз данных. Поддерживаются следующие варианты сравнения:
...
- Интерактивный выбор таблиц для сравнения через веб-интерфейс
...
- Экспорт результатов в HTML, CSV, JSON, TEXT форматы с возможностью скачивания
...
3. Выберите базовый отчет (файл)
4. Выберите целевой отчет (файл)
5. Нажмите "Загрузить и продолжить"
6. Выберите таблицы для сравнения из списка
7. Выберите формат вывода (HTML, TEXT, CSV, JSON)
8. Нажмите "Сравнить отчеты"
9. Просмотрите результаты на странице
10. Скачайте отчет кнопкой "Скачать отчет"
...
│   ├── select_tables.html         # Страница выбора таблиц
